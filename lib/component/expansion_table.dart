...
class _ExpansionTableState extends State<ExpansionTable> {
  ...
  Widget build(BuildContext context) {
    ...
    // List<Widget> _columns = _getColumns(columns: widget.data.header!);
    // final List<TableColumnWidth> tableColumns = List<TableColumnWidth>.filled(widget.data.header!.length + (displayCheckboxColumn ? 1 : 0), const _NullTableColumnWidth());
    final List<TableColumnWidth> tableColumns = List<TableColumnWidth>.filled(
        widget.data.header!.length, FixedColumnWidth(300.0));
    ...
    List<Widget> _columns = List.generate(widget.data.header!.length, (index) {
      final _column = widget.data.header![index];
      // double _paddingLeft = index > 0 ? effectiveColumnSpacing / 2 : effectiveColumnSpacing;
      // double _paddingRight =
      //     index < widget.data.header!.length ? effectiveColumnSpacing / 2 : effectiveColumnSpacing;
      return ClipRect(
    ...
    return LayoutBuilder(
      ...
      Widget getRows(
        ...
        List<Widget> _cellsBase = List.generate(row.cells.length, (index) {
          ...
          if (cellValue == null) {
            ...
            switch (_columnData.type) {
              ...
              case TableDataType.currency:
                _baseCell = Text(FormatData.currencyFormat().format(cellValue));
                break;
          ...
          // double _paddingRight = index < widget.data.header!.length
          //     ? effectiveColumnSpacing / 2
          //     : 0;
          if (index == 0) {
            double _childLeftSpace = 0;
            _childLeftSpace = (widget.data.level.toDouble()) * 16;
        ...
        if (widget.data.level.isEven) _rowDarker = !_rowDarker;
        ...
        if (row.child != null && row.active) {
          ...
          row.child!.level = (widget.data.level) + 1;
          row.child!.header = widget.data.header;
      ...
      List<Widget> _rows = List.generate(
        ...
        (index) => getRows(row: widget.data.rows[index], rowIndex: index),
      );

      if (widget.data.footer != null) {
        TableRowData footer = TableRowData(
          cells: widget.data.footer!,
        );
      ...
      double _headingRowHeight = widget.headingRowHeight ??
          ...
          56;
      double totalWidth =
          (widget.data.header!.length * 300) + effectiveHorizontalMargin;
